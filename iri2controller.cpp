...
void CIri2Controller::GoLoad ( unsigned int un_priority )
{
...
void CIri2Controller::SwitchBathroom ( unsigned int un_priority ){
...
/* We call vRepelent to go similar to Obstacle Avoidance, although it is an aproaching vector */
	...
	if(fBattToBathroom * fGoalToForageInhibitor==1.0){
		for(int i=0; i< m_seProx->GetNumberOfInputs(); i++){
			if(lightRed[i]!=0.0){
			//Activamos inhibitor
...
/******************************************************************************/
/******************************************************************************/
void CIri2Controller::DryFloor (unsigned int un_priority){
...
for(int i=0; i< m_seProx->GetNumberOfInputs(); i++){
	if(lightBlue[i]!=0.0){
		...
		if((fBathroomToDryInhibitor * fBattToDry * fGoalToForageInhibitor )==1.0){
		m_fActivationTable[un_priority][2] = 1.0;
...
void CIri2Controller::Forage ( unsigned int un_priority )
{
  ...
  m_fActivationTable[un_priority][1] = 1 - fMaxLight;
   ...
   /* If with a virtual puck */
	if ( ( groundMemory[0] * fGoalToForageInhibitor * fBattToForageInhibitor * fBathroomToForageInhibitor * fDryToForageInhibitor) == 1.0 )
	{
		/* Set Leds to BLUE */
